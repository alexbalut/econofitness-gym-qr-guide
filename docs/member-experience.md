...
Members never log in. `/` is the in-product gym home for **Éconofitness Montréal — Salaberry**.
...
3. Friendly URLs: `/m/econofitness-montreal/lat-pulldown`.
...
Keyed by gym slug (`econofitness-montreal`). No member account.

| Key | Contents |
|-----|----------|
| `econofitness-workout:v1:<slug>` | In-progress session JSON |
| `econofitness-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
